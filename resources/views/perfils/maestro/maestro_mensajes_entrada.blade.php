...
@section('cuerpo_mensajes')
		...
		@if(Auth::user()->maestro->mensajesRecibidos->isEmpty())
        ...
        <div class="table-responsive">
            <table id="tablaMensajes" class="table order-column" style="width:100%">
                ...
                <tbody id="body_tabla">
                    @foreach (Auth::user()->maestro->mensajesRecibidos as $mensaje)
					@if($mensaje->leido)
                    <tr  class='clickable_row' data-href="{{route('maestro_ver_recibido',$mensaje->id)}}" data-toggle="tooltip" data-placement="bottom" title="Ver mensaje">
					@else
					<tr  class='clickable_row' data-href="{{route('maestro_ver_recibido',$mensaje->id)}}" data-toggle="tooltip" style="background-color:#d3d0cb;" data-placement="bottom" title="Ver mensaje">
					@endif
