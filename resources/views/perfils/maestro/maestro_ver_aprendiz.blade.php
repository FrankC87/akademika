...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<li class="breadcrumb-item" aria-current="page"><a href="{{route('maestro_lista_aprendices')}}">Lista Aprendices</a></li>
	<li class="breadcrumb-item active" aria-current="page"><a href="{{route('maestro_ver_aprendiz',$aprendiz->id)}}">Aprendiz: {{ $aprendiz->nick_a }}</a></li>
  </ol>
</nav>
    ...
    <div class="row">
		...
		@foreach($aprendiz->maestros as $maestro)
				@if(Auth::user()->maestro->nick_m == $maestro->nick_m )
					<a href="{{route('maestro_expulsar',$aprendiz->id)}}" class='btn btn-danger align-middle'  style="padding: 10px;"><i class="fas fa-2x fa-sign-out-alt align-middle" style="font-size:20px;" ></i><span>&nbsp;&nbsp;EXPULSAR</span></a>		
				@endif
			@endforeach
  ...
  <div class="container px-3" id="perfil-visited-about">
    ...
    <div class="card mb-2 card_info">
        <div class="card-header font-weight-bold clickable_row" data-href="{{route('maestro_tema_ver',$comentario->tema->id)}}" data-toggle="tooltip" data-placement="bottom" title="Ver Articulo">
        ...
        <div class="card-footer">
            ...
            <span id="nºdislikes_{{$comentario->id}}" class="dislikes_comentario mx-2"><i class="fas fa-thumbs-down mx-2" data-toggle="tooltip" data-placement="bottom" title="No me gusta"></i>{{$comentario->dislikes}}</span>
			 <div class="font-weight-bold clickable_row float-right" data-href="{{route('maestro_ver_maestro',$comentario->tema->maestro->id)}}" data-toggle="tooltip" data-placement="bottom" title="Autor">
