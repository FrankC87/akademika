...
@section('cuerpo_mensajes')
		...
		@if(Auth::user()->aprendiz->mensajesRecibidos->isEmpty())
        ...
        <div>
            <table id="tablaMensajes" class="table order-column" style="width:100%">
                ...
                <tbody id="body_tabla">
                    @foreach (Auth::user()->aprendiz->mensajesRecibidos as $mensaje)
					@if($mensaje->leido)
					...
					@else
					<tr  class='clickable_row' data-href="{{route('aprendiz_ver_recibido',$mensaje->id)}}" data-toggle="tooltip" style="background-color:#d3d0cb;" data-placement="bottom" title="Ver mensaje">
					@endif
